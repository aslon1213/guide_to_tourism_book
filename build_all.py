...
UNIT_DIRS = [
    "unit-01-tourism-industry",
    "unit-02-hotel-accommodation",
    "unit-03-reservations",
    "unit-04-check-in-out",
    "unit-05-food-beverage",
    "unit-06-directions",
    "unit-07-complaints",
    "unit-08-telephone",
    "unit-09-tour-guiding",
    "unit-10-attractions",
    "unit-11-culture-etiquette",
    "unit-12-promoting",
    "unit-13-business-events",
    "unit-14-emergencies",
]
UNIT_FILES = [
    "01-opener.html",
    "02-warmup-vocabulary.html",
    "03-vocabulary.html",
    "04-grammar.html",
    "05-listening.html",
    "06-reading.html",
    "07-speaking-writing.html",
    "08-hometask.html",
]

reading_order = [
    ROOT / "front" / f
    for f in ("01-cover.html", "02-title.html", "03-preface.html", "04-contents.html")
]
for d in UNIT_DIRS:
...
def extract(path: Path) -> str:
    ...
    head_styles = "\n".join(
        re.findall(r"<style[^>]*>.*?</style>", html[: m.start()], re.S)
    )
    ...
    def _local(m):
        url = m.group(2)
        if re.match(
            r"^(?:shared/|assets/|front/|back/|units/|https?:|#|mailto:|data:)", url
        ):
            return m.group(0)
...
print(
    f"all.html written: {len(doc):,} bytes, {len(reading_order)} source files, {pages} .page sheets"
)
...
pages = doc.count('class="page ') + doc.count('class="page"')
print(
    f"index.html written: {len(doc):,} bytes, {len(reading_order)} source files, {pages} .page sheets"
)
